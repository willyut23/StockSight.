...
How to predict
Type a ticker symbol (e.g. MSFT) in the input field or click a quick‑pick button.

Choose a forecast horizon (1 Day or 5 Days).
...
🧠 How the Model Works
Data fetching – 6 months of daily OHLCV via yfinance.

Feature engineering – 10 lagged close prices, 3 lagged volumes, 5/10/20‑day moving averages, rolling standard deviation, and daily high‑low range.

Training – 80/20 time‑series split (no shuffling). Features are standardised.

Model – RandomForestRegressor (100 trees, max_depth=6).

Iterative forecasting – For multi‑day horizon, the model predicts one step ahead, then uses the predicted price to create the next day’s input features.

Confidence – R² score on the validation set is displayed to help you gauge reliability.
